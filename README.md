...
- **表情**：QQ 官方把表情解析为 `[表情]`/`[表情:赞]` 文本 + 图片附件。插件提取表情语义（如"用户发送了一个表情：[赞]"）一并挂起/注入，AI 能理解用户发了什么表情；自定义表情包（GIF）无法 OCR 的具体内容会以语义描述告知 AI（如"用户发送了一个表情包"）；`emoji_wait_pending` 开关可控制表情是否也参与"等待提问"。
...
- **v1.0.3**：支持平台表情（QQ 官方 `[表情]/[表情:赞]` + 图片附件）——
  - 提取表情语义（如"用户发送了一个表情：[赞]"）挂起/注入，AI 知道用户发了什么表情；
  - 自定义表情包（GIF）无法 OCR 的内容以语义描述告知 AI，不再报"识别失败"；
  - GIF 动图不进入 OCR，附占位说明（"GIF 动图，无法识别文字内容"）供 AI 知晓；
  - 新增 `emoji_wait_pending` 开关：控制表情是否也参与「发送后等待提问」门控。
...
- **发了表情（尤其是自定义表情包 GIF）后机器人不回复**：表情也会进入「等待提问」模式（默认开启）——发表情后需再发文字问题，AI 才会结合表情语义回答；若希望表情立即被正常回复，将 `emoji_wait_pending` 关闭即可。
- **GIF 动图识别不了文字**：动图不适合 OCR，插件会告知 AI"用户发送了 GIF 动图"，AI 至少知道用户发了动图；静态截图请发 PNG/JPG。
